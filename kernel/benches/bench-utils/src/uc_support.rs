...
/// Returns (engine, table_url, path_prefix, object_store) where:
/// - engine: Delta Kernel engine with UC credentials
/// - table_url: Full table URL
/// - path_prefix: Path within the object store for writing
/// - object_store: Arc to the object store for direct file operations
...
> {
    ...
    // Wrap store in Arc so we can use it both for the engine and for file operations
    let store_arc: Arc<dyn object_store::ObjectStore> = Arc::from(store);
    let store_for_files = Arc::clone(&store_arc);
    ...
    let engine = Arc::new(
        delta_kernel::engine::default::DefaultEngine::new_with_executor(store_arc, executor),
    );

    // Return both engine and table_url, plus the path prefix for writing and the store
    ...
    Ok((engine, table_url, path_prefix, store_for_files))
}
...
pub async fn setup_table_access(
    ...
    if using_uc {
        ...
        let (engine, table_url, path_prefix, _store) =
