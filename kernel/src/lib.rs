...
pub use content_tree::{ContentTreeNodeEntry, DataContentType, TrackingInfo, TrackingStatus};
...
pub trait EvaluationHandler: AsAny {
    ...
    ///
    /// Each element in `rows` represents one row of data, where each row is a slice of
    /// structured scalar values (one scalar per top-level field in the schema).
    ///
    /// # Parameters
    ///
    ...
    /// # Example
    ///
    /// For a schema with fields `[add: Struct, remove: Struct]`, each row should contain
    /// exactly 2 scalars: one for the `add` field and one for the `remove` field.
    fn create_many(
        &self,
        schema: SchemaRef,
        rows: &[&[Scalar]],
    ...
    /// Create a new [`LookupJoiner`] for performing efficient key-based joins with cached lookup data.
    ///
    ...
    /// - `value_columns`: Column paths of the values to extract from lookup data and append during join.
    /// - `lookup_version_column`: Column path for version in lookup data. **Must be Int64 type and must not contain null values**.
