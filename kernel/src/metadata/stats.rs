...
/// Converts an Arrow StructArray in Delta stats format to AMT (Adaptive Metadata Tree) format.
///
/// This is a thin wrapper that computes the AMT schema and delegates to
/// [`crate::engine::arrow_utils::pivot_delta_stats_to_amt`] for the zero-copy pivot.
#[cfg(any(
    feature = "default-engine-base",
    feature = "default-engine-rustls",
    feature = "arrow-conversion"
))]
pub(crate) fn convert_delta_stats_to_amt(
    delta_stats: &crate::arrow::array::StructArray,
    table_schema: &StructType,
) -> DeltaResult<crate::arrow::array::StructArray> {
    let amt_schema = stats_schema(table_schema)?;
    crate::engine::arrow_utils::pivot_delta_stats_to_amt(delta_stats, table_schema, &amt_schema)
}
