...
impl Snapshot {
    ...
    fn try_new_from_impl(
        ...
        let (new_metadata, new_protocol, new_checkpoint_action) = new_log_segment
            .protocol_and_metadata_and_checkpoint(engine, Some(existing_protocol), &lazy_crc)?;
    ...
    pub(crate) fn try_new_from_log_segment_impl(
        location: Url,
        log_segment: LogSegment,
        engine: &dyn Engine,
        operation_id: MetricId,
    ...
    pub(crate) fn get_file_stats(&self, engine: &dyn Engine) -> Option<FileStats> {
        self.get_or_load_file_stats(engine)
    }
