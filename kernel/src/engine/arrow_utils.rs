...
///
/// If the data is Arrow-backed and contains a stats column in Delta JSON format
/// (with `numRecords` field), this converts it to AMT format zero-copy using
/// [`crate::metadata::stats::convert_delta_stats_to_amt`]. Returns a new `EngineData`
/// with the converted column, or `None` if no conversion was needed.
pub(crate) fn try_pre_convert_stats_column(
    data: &dyn EngineData,
    stats_column_name: &str,
    table_schema: &StructType,
) -> DeltaResult<Option<Box<dyn EngineData>>> {
    // 1. Downcast to ArrowEngineData — return None if not Arrow
    let Some(arrow_data) = data.any_ref().downcast_ref::<ArrowEngineData>() else {
        return Ok(None);
    };

    let batch = arrow_data.record_batch();

    // 2. Find the stats column by name — return None if missing
    let Some(col_idx) = batch.schema().index_of(stats_column_name).ok() else {
        return Ok(None);
    };

    // 3. Downcast to StructArray, check for numRecords field (Delta JSON format indicator)
    let Some(stats_struct) = batch.column(col_idx).as_any().downcast_ref::<StructArray>() else {
        return Ok(None);
    };

    // If there's no "numRecords" field, this is already AMT format — no conversion needed
    if stats_struct.column_by_name("numRecords").is_none() {
        return Ok(None);
    }

    // 4. Convert Delta JSON stats to AMT format
    let amt_struct =
        crate::metadata::stats::convert_delta_stats_to_amt(stats_struct, table_schema)?;

    // 5. Build a new RecordBatch with the converted column and updated schema field
    let mut columns: Vec<ArrayRef> = batch.columns().to_vec();
    let amt_array: ArrayRef = Arc::new(amt_struct);

    // Build a new schema field for the converted stats column
    let new_field = ArrowField::new(
        stats_column_name,
        amt_array.data_type().clone(),
        batch.schema().field(col_idx).is_nullable(),
    );

    let mut fields: Vec<ArrowFieldRef> =
        batch.schema().fields().iter().cloned().collect::<Vec<_>>();
    fields[col_idx] = Arc::new(new_field);
    columns[col_idx] = amt_array;

    let new_schema = Arc::new(ArrowSchema::new(fields));
    let new_batch = RecordBatch::try_new(new_schema, columns)?;

    Ok(Some(Box::new(ArrowEngineData::new(new_batch))))
}

/// Pivots a Delta-format stats StructArray into AMT (Adaptive Metadata Tree) per-column format.
///
/// Delta stores stats per category:
///   `{numRecords, minValues: {col1, col2}, maxValues: {col1, col2}, nullCount: {col1, col2}, tightBounds}`
///
/// AMT stores stats per column:
///   `{col1: {value_count, lower_bound, upper_bound, ...}, col2: {...}}`
///
/// This function restructures the data zero-copy by rearranging `Arc<dyn Array>` references —
/// no buffer copying, no per-row scalar extraction.
#[allow(dead_code)]
pub(crate) fn pivot_delta_stats_to_amt(
    delta_stats: &StructArray,
    table_schema: &StructType,
    amt_stats_schema: &StructType,
) -> DeltaResult<StructArray> {
    let num_rows = delta_stats.len();
    let null_buffer = delta_stats.nulls().cloned();

    // Extract top-level source arrays from the delta stats struct
    let num_records = delta_stats.column_by_name("numRecords");
    let min_values = delta_stats
        .column_by_name("minValues")
        .and_then(|a| a.as_any().downcast_ref::<StructArray>());
    let max_values = delta_stats
        .column_by_name("maxValues")
        .and_then(|a| a.as_any().downcast_ref::<StructArray>());
    let null_count = delta_stats
        .column_by_name("nullCount")
        .and_then(|a| a.as_any().downcast_ref::<StructArray>());
    let tight_bounds = delta_stats.column_by_name("tightBounds");

    // Recursively build AMT-format columns
    let (fields, columns) = build_amt_struct_columns(
        table_schema,
        amt_stats_schema,
        num_records,
        min_values,
    ...
    Ok(StructArray::try_new(fields.into(), columns, null_buffer)?)
}
...
fn build_amt_struct_columns(
    table_schema: &StructType,
    amt_stats_schema: &StructType,
    num_records: Option<&ArrayRef>,
    min_values: Option<&StructArray>,
    max_values: Option<&StructArray>,
    null_count: Option<&StructArray>,
    tight_bounds: Option<&ArrayRef>,
    num_rows: usize,
) -> DeltaResult<(Vec<ArrowFieldRef>, Vec<ArrayRef>)> {
    let mut fields = Vec::new();
    let mut columns = Vec::new();

    for amt_field in amt_stats_schema.fields() {
        let arrow_field: ArrowField = amt_field.try_into_arrow()?;
        let table_field = table_schema.field(amt_field.name());

        let column: ArrayRef = if let Some(tf) = table_field {
            match tf.data_type() {
                DataType::Struct(nested_table_struct) => {
                    // Table field is a struct — recurse into nested StructArrays
                    let nested_amt_struct = match amt_field.data_type() {
                        DataType::Struct(s) => s.as_ref(),
                        _ => {
                            return Err(Error::internal_error(
                                "AMT schema for struct table field should be struct",
                            ));
                        }
                    };

                    let nested_min = min_values
                        .and_then(|mv| mv.column_by_name(amt_field.name()))
                        .and_then(|a| a.as_any().downcast_ref::<StructArray>());
                    let nested_max = max_values
                        .and_then(|mv| mv.column_by_name(amt_field.name()))
                        .and_then(|a| a.as_any().downcast_ref::<StructArray>());
                    let nested_null = null_count
                        .and_then(|nc| nc.column_by_name(amt_field.name()))
                        .and_then(|a| a.as_any().downcast_ref::<StructArray>());

                    let (nested_fields, nested_columns) = build_amt_struct_columns(
                        nested_table_struct,
                        nested_amt_struct,
                        num_records,
                        nested_min,
                ...
                _ => {
                    // Table field is a primitive — build per-column leaf stats
                    let amt_leaf_struct = match amt_field.data_type() {
                        DataType::Struct(s) => s.as_ref(),
                        _ => {
                            return Err(Error::internal_error(
                                "AMT schema for primitive table field should be struct",
                            ));
                        }
                    };

                    let min_col = min_values.and_then(|mv| mv.column_by_name(amt_field.name()));
                    let max_col = max_values.and_then(|mv| mv.column_by_name(amt_field.name()));
                    let null_col = null_count.and_then(|nc| nc.column_by_name(amt_field.name()));

                    Arc::new(build_amt_leaf_stats(
                        amt_leaf_struct,
                        num_records,
                        min_col,
                        max_col,
            ...
            // Column missing from table schema — produce null array
            new_null_array(arrow_field.data_type(), num_rows)
        };
...
/// Assembles a per-column stats StructArray from source arrays.
///
/// Uses `Arc::clone()` for zero-copy sharing and `new_null_array()` for stats
/// not present in the Delta format (nan_value_count, avg_value_size, max_value_size).
fn build_amt_leaf_stats(
    amt_stats_struct: &StructType,
    num_records: Option<&ArrayRef>,
    min_col: Option<&ArrayRef>,
    max_col: Option<&ArrayRef>,
    null_col: Option<&ArrayRef>,
    tight_bounds: Option<&ArrayRef>,
    num_rows: usize,
) -> DeltaResult<StructArray> {
    let mut fields = Vec::new();
    ...
    for stats_field in amt_stats_struct.fields() {
        let arrow_field: ArrowField = stats_field.try_into_arrow()?;

        let column: ArrayRef = match stats_field.name().as_str() {
            "value_count" => num_records
                .map(Arc::clone)
                .unwrap_or_else(|| new_null_array(arrow_field.data_type(), num_rows)),
            "null_value_count" => null_col
                .map(Arc::clone)
                .unwrap_or_else(|| new_null_array(arrow_field.data_type(), num_rows)),
            "lower_bound" => min_col
                .map(Arc::clone)
            ...
            "upper_bound" => max_col
                .map(Arc::clone)
                .unwrap_or_else(|| new_null_array(arrow_field.data_type(), num_rows)),
            "exact_bounds" => tight_bounds
                .map(Arc::clone)
                .unwrap_or_else(|| new_null_array(arrow_field.data_type(), num_rows)),
            // nan_value_count, avg_value_size, max_value_size: not in Delta stats
            _ => new_null_array(arrow_field.data_type(), num_rows),
        };
    ...
    Ok(StructArray::try_new(fields.into(), columns, None)?)
}
...
"#;
    ...
    // --- Tests for pivot_delta_stats_to_amt ---

    use crate::arrow::array::Int64Array;

    /// Helper to create a StructField with parquet field ID metadata (mirrors stats.rs field_with_id).
    fn field_with_id(
        name: &str,
        data_type: DataType,
        nullable: bool,
    ...
    /// Build a Delta-format stats StructArray from components.
    fn build_delta_stats_struct(
        num_records: Arc<dyn ArrowArray>,
        min_values: Arc<dyn ArrowArray>,
        max_values: Arc<dyn ArrowArray>,
        null_count: Arc<dyn ArrowArray>,
        tight_bounds: Arc<dyn ArrowArray>,
    ) -> StructArray {
        let fields: Vec<Arc<ArrowField>> = vec![
    ...
    #[test]
    fn test_pivot_delta_stats_basic() {
        // Table schema: id (long, non-nullable, field_id=1), name (string, nullable, field_id=2)
        let table_schema = StructType::new_unchecked([
            field_with_id("id", DataType::LONG, false, 1),
            field_with_id("name", DataType::STRING, true, 2),
        ]);
        let amt_schema =
            crate::metadata::stats::stats_schema(&table_schema).expect("stats_schema should work");

        // Build Delta stats: 2 rows
        let num_records: Arc<dyn ArrowArray> = Arc::new(Int64Array::from(vec![100, 200]));
        let min_values: Arc<dyn ArrowArray> = Arc::new(
            StructArray::try_new(
                vec![
                    Arc::new(ArrowField::new("id", ArrowDataType::Int64, true)),
                    Arc::new(ArrowField::new("name", ArrowDataType::Utf8, true)),
                ]
                .into(),
                vec![
                    Arc::new(Int64Array::from(vec![1, 10])) as _,
        ...
        let max_values: Arc<dyn ArrowArray> = Arc::new(
            StructArray::try_new(
                vec![
                    Arc::new(ArrowField::new("id", ArrowDataType::Int64, true)),
                    Arc::new(ArrowField::new("name", ArrowDataType::Utf8, true)),
                ...
                .into(),
                vec![
                    Arc::new(Int64Array::from(vec![50, 100])) as _,
                    Arc::new(StringArray::from(vec!["zoe", "max"])) as _,
                ],
                None,
            )
            .unwrap(),
        );
        let null_count: Arc<dyn ArrowArray> = Arc::new(
            StructArray::try_new(
                vec![
                    Arc::new(ArrowField::new("id", ArrowDataType::Int64, true)),
                    Arc::new(ArrowField::new("name", ArrowDataType::Int64, true)),
                ]
                .into(),
                vec![
                    Arc::new(Int64Array::from(vec![0, 0])) as _,
                    Arc::new(Int64Array::from(vec![5, 10])) as _,
                ],
                None,
            )
            .unwrap(),
        );
        let tight_bounds: Arc<dyn ArrowArray> = Arc::new(BooleanArray::from(vec![true, false]));

        let delta_stats = build_delta_stats_struct(
            num_records.clone(),
            min_values,
            max_values,
            null_count,
        ...
        let result = pivot_delta_stats_to_amt(&delta_stats, &table_schema, &amt_schema).unwrap();

        assert_eq!(result.len(), 2);
        assert_eq!(result.num_columns(), 2);

        // Check "id" column stats
        let id_stats = result.column_by_name("id").unwrap();
        let id_struct = id_stats.as_any().downcast_ref::<StructArray>().unwrap();
        // Non-nullable id: value_count, lower_bound, upper_bound, exact_bounds (4 fields)
        assert_eq!(id_struct.num_columns(), 4);

        let id_value_count = id_struct.column_by_name("value_count").unwrap();
        let id_lower = id_struct.column_by_name("lower_bound").unwrap();
        let id_upper = id_struct.column_by_name("upper_bound").unwrap();
        let id_exact = id_struct.column_by_name("exact_bounds").unwrap();
        ...
        let name_struct = name_stats.as_any().downcast_ref::<StructArray>().unwrap();
        // Nullable string: value_count, null_value_count, avg_value_size, max_value_size,
        //                   lower_bound, upper_bound, exact_bounds (7 fields)
        assert_eq!(name_struct.num_columns(), 7);

        let name_lower = name_struct.column_by_name("lower_bound").unwrap();
        let name_upper = name_struct.column_by_name("upper_bound").unwrap();
        let name_null_count = name_struct.column_by_name("null_value_count").unwrap();
        ...
        // avg_value_size and max_value_size should be null arrays
        let avg_size = name_struct.column_by_name("avg_value_size").unwrap();
        assert_eq!(avg_size.null_count(), 2); // all nulls
        let max_size = name_struct.column_by_name("max_value_size").unwrap();
        assert_eq!(max_size.null_count(), 2); // all nulls
    }

    #[test]
    fn test_pivot_delta_stats_zero_copy() {
        // Verify that shared arrays (numRecords → value_count, tightBounds → exact_bounds)
        // are truly zero-copy via Arc::ptr_eq.
        let table_schema = StructType::new_unchecked([
        ...
        let num_records: Arc<dyn ArrowArray> = Arc::new(Int64Array::from(vec![42]));
        let tight_bounds: Arc<dyn ArrowArray> = Arc::new(BooleanArray::from(vec![true]));

        let min_values: Arc<dyn ArrowArray> = Arc::new(
            StructArray::try_new(
                vec![
                    Arc::new(ArrowField::new("a", ArrowDataType::Int64, true)),
                    Arc::new(ArrowField::new("b", ArrowDataType::Int32, true)),
                ]
                .into(),
                vec![
        ...
        let max_values: Arc<dyn ArrowArray> = Arc::new(
            StructArray::try_new(
                ...
                .into(),
                vec![
                    Arc::new(Int64Array::from(vec![99])) as _,
                    Arc::new(Int32Array::from(vec![50])) as _,
                ],
                None,
            )
            .unwrap(),
        ...
        let null_count: Arc<dyn ArrowArray> = Arc::new(
            StructArray::try_new(
                vec![
                    Arc::new(ArrowField::new("a", ArrowDataType::Int64, true)),
                    Arc::new(ArrowField::new("b", ArrowDataType::Int64, true)),
                ]
                .into(),
                vec![
        ...
        // Both "a" and "b" should share the same numRecords array for value_count
        let a_stats = result
            .column_by_name("a")
            .unwrap()
            .as_any()
        ...
        let a_value_count = a_stats.column_by_name("value_count").unwrap();
        let b_value_count = b_stats.column_by_name("value_count").unwrap();
        assert!(Arc::ptr_eq(a_value_count, b_value_count));
        // And they should be the same as the original numRecords
        assert!(Arc::ptr_eq(a_value_count, &num_records));

        // Both should share the same tightBounds array for exact_bounds
        let a_exact = a_stats.column_by_name("exact_bounds").unwrap();
        let b_exact = b_stats.column_by_name("exact_bounds").unwrap();
        assert!(Arc::ptr_eq(a_exact, b_exact));
        assert!(Arc::ptr_eq(a_exact, &tight_bounds));
    }

    #[test]
    fn test_pivot_delta_stats_nested_struct() {
        // Table: outer (struct { inner_id: long, inner_name: string })
        let inner_struct = StructType::new_unchecked([
            field_with_id("inner_id", DataType::LONG, false, 2),
            field_with_id("inner_name", DataType::STRING, true, 3),
        ]);
        ...
        // Delta stats with nested minValues/maxValues/nullCount
        let num_records: Arc<dyn ArrowArray> = Arc::new(Int64Array::from(vec![100]));
        let tight_bounds: Arc<dyn ArrowArray> = Arc::new(BooleanArray::from(vec![true]));

        let inner_min = StructArray::try_new(
            vec![
                Arc::new(ArrowField::new("inner_id", ArrowDataType::Int64, true)),
                Arc::new(ArrowField::new("inner_name", ArrowDataType::Utf8, true)),
            ]
            .into(),
            vec![
                Arc::new(Int64Array::from(vec![1])) as _,
        ...
        let min_values: Arc<dyn ArrowArray> = Arc::new(
            StructArray::try_new(
                vec![Arc::new(ArrowField::new_struct(
                    "outer",
                    inner_min.fields().clone(),
                    true,
                ))]
                .into(),
                vec![Arc::new(inner_min) as _],
                None,
            )
            .unwrap(),
        );

        let inner_max = StructArray::try_new(
            vec![
                Arc::new(ArrowField::new("inner_id", ArrowDataType::Int64, true)),
                Arc::new(ArrowField::new("inner_name", ArrowDataType::Utf8, true)),
            ]
            ...
            vec![
                ...
                Arc::new(StringArray::from(vec!["zoe"])) as _,
            ],
            None,
        )
        .unwrap();
        let max_values: Arc<dyn ArrowArray> = Arc::new(
            StructArray::try_new(
                vec![Arc::new(ArrowField::new_struct(
                    "outer",
                    inner_max.fields().clone(),
                    true,
                ))]
                .into(),
                vec![Arc::new(inner_max) as _],
                None,
            )
            .unwrap(),
        );

        let inner_null = StructArray::try_new(
            vec![
                Arc::new(ArrowField::new("inner_id", ArrowDataType::Int64, true)),
                Arc::new(ArrowField::new("inner_name", ArrowDataType::Int64, true)),
            ]
            .into(),
            vec![
                Arc::new(Int64Array::from(vec![0])) as _,
                Arc::new(Int64Array::from(vec![5])) as _,
            ],
            None,
        )
        .unwrap();
        let null_count: Arc<dyn ArrowArray> = Arc::new(
            StructArray::try_new(
                vec![Arc::new(ArrowField::new_struct(
                    "outer",
                    inner_null.fields().clone(),
                    true,
                ))]
                .into(),
                vec![Arc::new(inner_null) as _],
                None,
            )
            .unwrap(),
        );
        ...
        // AMT: {outer: {inner_id: {value_count, ...}, inner_name: {value_count, ...}}}
        let outer_stats = result
            .column_by_name("outer")
            .unwrap()
            .as_any()
            .downcast_ref::<StructArray>()
            .unwrap();
        assert_eq!(outer_stats.num_columns(), 2);

        let inner_id_stats = outer_stats
        ...
        let inner_name_stats = outer_stats
    ...
    #[test]
    fn test_pivot_delta_stats_missing_columns() {
        // Delta stats has minValues/maxValues with only "id", but table schema also has "name".
        // The "name" stats should be null arrays.
        let table_schema = StructType::new_unchecked([
            field_with_id("id", DataType::LONG, false, 1),
            field_with_id("name", DataType::STRING, true, 2),
        ]);
        ...
        // Only "id" in minValues/maxValues/nullCount — "name" is missing
        let min_values: Arc<dyn ArrowArray> = Arc::new(
            StructArray::try_new(
                vec![Arc::new(ArrowField::new("id", ArrowDataType::Int64, true))].into(),
        ...
        let max_values: Arc<dyn ArrowArray> = Arc::new(
            StructArray::try_new(
                vec![Arc::new(ArrowField::new("id", ArrowDataType::Int64, true))].into(),
                vec![Arc::new(Int64Array::from(vec![50])) as _],
                None,
            )
            .unwrap(),
        );
        let null_count: Arc<dyn ArrowArray> = Arc::new(
            StructArray::try_new(
                vec![Arc::new(ArrowField::new("id", ArrowDataType::Int64, true))].into(),
                vec![Arc::new(Int64Array::from(vec![0])) as _],
                None,
            )
            .unwrap(),
        );
        ...
        // "name" stats should exist but lower_bound/upper_bound/null_value_count should be null
        let name_stats = result
            .column_by_name("name")
            .unwrap()
            .as_any()
            .downcast_ref::<StructArray>()
            .unwrap();
        let name_lower = name_stats.column_by_name("lower_bound").unwrap();
        assert_eq!(name_lower.null_count(), 1); // all nulls
        let name_upper = name_stats.column_by_name("upper_bound").unwrap();
        assert_eq!(name_upper.null_count(), 1); // all nulls
        let name_null_count = name_stats.column_by_name("null_value_count").unwrap();
        assert_eq!(name_null_count.null_count(), 1); // all nulls

        // But value_count and exact_bounds should still have values (shared from top-level)
        let name_value_count = name_stats.column_by_name("value_count").unwrap();
        assert_eq!(name_value_count.null_count(), 0);
    }

    #[test]
    fn test_pivot_delta_stats_empty_batch() {
        let table_schema =
        ...
        // 0 rows
        let num_records: Arc<dyn ArrowArray> = Arc::new(Int64Array::from(Vec::<i64>::new()));
        let tight_bounds: Arc<dyn ArrowArray> = Arc::new(BooleanArray::from(Vec::<bool>::new()));
        let min_values: Arc<dyn ArrowArray> = Arc::new(
            StructArray::try_new(
                vec![Arc::new(ArrowField::new("id", ArrowDataType::Int64, true))].into(),
