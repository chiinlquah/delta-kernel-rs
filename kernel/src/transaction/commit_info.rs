...
fn commit_info_literal_exprs(
    ...
    let literal_exprs = vec![
        ...
        (
            "snapshotId",
            Arc::new(Expression::literal(commit_info.snapshot_id)),
        ),
    ];
    ...
    fn test_build_commit_info_full_overlap() -> DeltaResult<()> {
        ...
        let (data, schema) = make_engine_commit_info(
            vec![
                ...
                ArrowField::new("snapshotId", ArrowDataType::Int64, true),
            ],
            vec![
                ...
                Arc::new(Int64Array::from(vec![Some(999i64)])) as ArrayRef,
            ],
        );
        let (engine, txn) = make_txn(Some((data, schema)))?;
        ...
        // All 9 CommitInfo fields are present in the engine schema -- no fields appended.
        assert_eq!(commit_info.num_columns(), 9);
        ...
        assert_eq!(get_str(commit_info, "engineInfo"), "test_engine/1.0");
        assert!(!get_bool(commit_info, "isBlindAppend"));
        assert_eq!(get_i64(commit_info, "snapshotId"), 0);
