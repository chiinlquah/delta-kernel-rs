...
/// Approximate Snappy-compressed bytes per row for `(int32, 100-char padded string)` parquet
/// data. The padded strings ("000...001") are highly compressible, so this is the post-compression
/// estimate rather than the uncompressed row size.
const APPROX_BYTES_PER_ROW: i64 = 20;

/// Row count guaranteed to produce a parquet file exceeding [`FIRST_BIN_BOUNDARY`].
/// Uses 4x the boundary divided by per-row size as a safety margin against compression variance.
const LARGE_FILE_ROW_COUNT: i32 = (FIRST_BIN_BOUNDARY * 4 / APPROX_BYTES_PER_ROW) as i32;
