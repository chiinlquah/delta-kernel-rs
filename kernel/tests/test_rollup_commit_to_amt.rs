...
fn field_with_metadata(name: &str, data_type: DataType, field_id: i64) -> StructField {
    StructField::nullable(name, data_type).with_metadata([
        (ColumnMetadataKey::ColumnMappingId.as_ref(), MetadataValue::Number(field_id)),
        (ColumnMetadataKey::ColumnMappingPhysicalName.as_ref(), MetadataValue::String(name.to_string())),
    ])
}
...
async fn test_batch_commit_no_op_when_up_to_date() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create table
    create_table(store.clone(), table_url.clone(), schema.clone(), &[], true,
        vec!["columnMapping", "metadataTree-experimental"],
        vec!["columnMapping", "metadataTree-experimental"]).await?;
...
"#;
    let commit_path = object_store::path::Path::from(format!("no_op_batch_commit/_delta_log/{:020}.json", 1));
    store.put(&commit_path, commit_json.as_bytes().to_vec().into()).await?;

    // Batch commit to create content root
    let snapshot = Snapshot::builder_for(table_url.clone()).build(engine.as_ref())?;
    println!("Snapshot version before first batch commit: {}", snapshot.version());

    let txn = snapshot.transaction(Box::new(FileSystemCommitter::new()), engine.as_ref())?.with_batch_commit();
    let _first_commit_result = txn.commit(engine.as_ref())?;

    // Verify content root was created
    let snapshot = Snapshot::builder_for(table_url.clone()).build(engine.as_ref())?;
    assert!(snapshot.content_root().is_some(), "Content root should exist after first batch commit");
    let content_root_version = snapshot.content_root().unwrap().version();
    println!("After first batch commit - snapshot version: {}, content root version: {}",
             snapshot.version(), content_root_version);
    ...
    let new_commit_version = if let delta_kernel::transaction::CommitResult::CommittedTransaction(committed) = result {
        committed.commit_version()
    } else {
        panic!("Expected committed transaction");
    };
    println!("Second batch commit created version: {}", new_commit_version);
    ...
    println!("After second batch commit - snapshot version: {}, content root version: {:?}",
             new_snapshot.version(), new_content_root_version);

    // The content root should still point to the same version (no rebuild occurred)
    assert_eq!(new_content_root_version, Some(content_root_version),
