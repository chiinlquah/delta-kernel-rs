...
mod starts_with {
    ...
    pub(super) fn range_bounds(
        ...
        // skips when `[min, max]` lies entirely outside that range), so v1
        // opts out and engines fall back to row-level filtering.
        if inverted {
            return None;
        }
...
/// to-last is incremented; this recurses through any trailing run of
/// `U+10FFFF`.
...
mod tests {
    ...
    //
    // The indirect path (file pruning over a rewritten predicate) gets full
    // coverage via the integration tests. The direct path (`Output = bool`,
    // evaluated against per-row-group stats) is otherwise untested -- this
    // exercises the same generic via the other associated type.
...
//
// Drives a real scan against an in-memory Delta log to prove `STARTS_WITH`
// pruning fires through the full kernel data-skipping pipeline -- the unit
// tests above cover the rewrite logic, but only this exercise can catch a
// regression where the engine plumbing fails to consult our op.
...
mod integration_tests {
    ...
    async fn starts_with_keeps_all_files_when_inverted() {
        // NOT STARTS_WITH opts out of skipping in v1, so every file must
        // survive the metadata-level filter even when its stats clearly
        // don't satisfy the inverted predicate.
        let (engine, storage) = engine_and_storage();
        write_table(
            &storage,
            &[
        ...
        assert_eq!(
            ...
            "inverted STARTS_WITH should not prune in v1"
        );
    }
    ...
    async fn opt_out_opaque_under_and_still_prunes_via_native_sibling() {
        ...
        // to skip files. Exercises the fall-through path in
        // rewrite_for_file_pruning -- the path the scalar pruning test
        // covers but the integration tests otherwise don't.
        let (engine, storage) = engine_and_storage();
        write_table(
            &storage,
            &[
