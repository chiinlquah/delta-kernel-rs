...
/// `skipping` shim (currently `STARTS_WITH`) participate in partition pruning, stats-based
/// file pruning, and parquet row-group skipping. Unrecognized names opt out of every
/// pruning pass, and the engine is responsible for filtering them at row time.
///
